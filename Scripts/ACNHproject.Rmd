...
## Setting up color vectors for entire code
```{r Colors}
...
## Common User Words:    
When running this chunk, it yields a tibble table where the words are descending based on the average grade (1-10) from the *user reviews*. For some odd reason the word **"bombing"** was associated with high average reviews. I looked further into why because there are no **bombs** in the game so why are people saying it in the reviews?
```{r User Reviews TA}
...
head(bombing_reviews$text) #view the top part of the bombing reviews text
```
...
wordcloud2(criticloud, shape = 'star', size = .3, minSize = 5, rotateRatio=0, shuffle = FALSE, color = rep_len(nook.colors, nrow(criticloud)), backgroundColor = nook.background) # this doesnt show when knit but does show in output folder and as runned chunk
```

  After creating both word clouds, I am still having trouble trying to determine what exactly is the difference between the user and critic clouds. The cloluds look relatively similar, both have some matching words such as, "fun, experience, crafting, and time." So I beg the question . . .
...
user_sent_word_counts <- user_review_words %>% #calling in user review words and wrangling more for the sentiment analysis, called it someting new
...
user_reviews %>% 
  ggplot(aes(grade))+
  geom_histogram(fill = "#7fa9ff", color= "#000000")+
      labs(x = "Average Grade Out of 10",y = "Number of Reviews",
       title = "User Review Ratings in 2020 for ACNH ", caption = "By: Kevin Candray")+ #labels
  #scale_fill_manual(values = c("#74bfd4", "#03a279"))+
  theme_bw()+
  theme(axis.text = element_text(size=12),
        axis.title = element_text(size=16),
...
user_reviews %>% 
  ...
  geom_point(aes(size=nb_reviews),
             color = "#0123ac")+
  expand_limits(y=0)+
    labs(x = "Date in 2020",y = "Average Grade Out of 10",
       title = "User Average Review Ratings in 2020 for ACNH ", caption = "By: Kevin Candray")+ #labels
  #scale_fill_manual(values = c("#74bfd4", "#03a279"))+
  theme_bw()+
  theme(axis.text = element_text(size=12),
        axis.title = element_text(size=16),
...
critic %>% 
  ...
  geom_point(aes(size=nb_reviews),
             color = "#64ba60")+
  expand_limits(y=0)+
    labs(x = "Date in 2020",y = "Average Grade Out of 100",
       title = "User Average Review Ratings in 2020 for ACNH ", caption = "By: Kevin Candray")+ #labels
  #scale_fill_manual(values = c("#74bfd4", "#03a279"))+
  theme_bw()+
  theme(axis.text = element_text(size=12),
